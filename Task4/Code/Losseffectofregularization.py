...
cost2 = lambda w, reg: .5 * np.mean((w[1] + w[0] * x - y) ** 2) + reg * l2_penalty(w[0])
...
for i, reg_coef in enumerate(reg_list):
    ...
    optimizer = GradientDescent(learning_rate=.001, max_iters=10000, record_history=True)
...
cost1 = lambda w, reg: .5 * np.mean((w[1] + w[0] * x - y) ** 2) + reg * l1_penalty(w[0])
for i, reg_coef in enumerate(reg_list):
    fig, axes = plt.subplots(ncols=1, nrows=1, constrained_layout=True, figsize=(10, 10))
    optimizer = GradientDescent(learning_rate=.001, max_iters=10000, record_history=True)
